package com.xin.dev.modulex.processor
...
class ModuleXProcessor(private val environment: SymbolProcessorEnvironment) : SymbolProcessor {
    ...
    inner class ModuleKSVisitor : KSVisitorVoid() {
        override fun visitClassDeclaration(classDeclaration: KSClassDeclaration, data: Unit) {
            ...
            if (!classDeclaration.superTypes.any {
                    it.resolve().declaration.qualifiedName?.asString() == "com.xin.dev.modulex.api.IModule"
                }) {
            ...
            val fileContent = buildString {
                ...
                appendLine("import com.xin.dev.modulex.proxy.Creator")
                appendLine("import com.xin.dev.modulex.proxy.ModuleProxy")
                appendLine()
                // 生成类声明
    ...
    inner class AppKSVisitor : KSVisitorVoid() {
        ...
        override fun visitClassDeclaration(classDeclaration: KSClassDeclaration, data: Unit) {
            ...
            val fileContent = buildString {
                appendLine("package $proxyClassPackageName")
                appendLine()
                appendLine("import com.xin.dev.modulex.proxy.AppProxy")
                appendLine("import com.xin.dev.modulex.proxy.ModuleProxy")
                appendLine()
    ...
    companion object {
        private const val NAME_MODULE_PACKAGE = "com.xin.dev.modulex.modpxy"
